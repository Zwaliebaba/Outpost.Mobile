...
// root only; a file read there is exactly that, and not an exception to it (ADR 0045). Libraries go
...
struct ServerConfig
{
  ...
  // Which shard this world mints identities for (ADR 0044). Zero is right for one process; two
  // servers that both minted shard 0 would issue colliding ids, which is the one value here that
  // is wrong to leave at its default on a second machine.
  Game::ShardId shard = 0;
