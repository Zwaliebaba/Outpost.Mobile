...
it needs no SDK and no Windows runner. `clang-tidy` runs on Windows after the build and **gates**
over GameLogic and NeuronServer. Each arrived non-blocking and was promoted once runs on the
...
| `GameLogic/` | The deterministic simulation, namespace `Game`. `Universe`, `ShipState`, `UniversePos`, `HullSpec`, `DeviceSpec` (what a gun is; `HullSpec` says where a hull carries one), `Movement`, `Collision`, `SpatialIndex`, `PathGrid`, `Formation`, `Patrol`, `SimTuning`, `InterestSet`, `PathIslands` (the architecture partitioned into islands, one `PathGrid` over each, ADR 0033), `UniverseLayout` (a solar system's star and planet sites from a seed — static content both halves read, ADR 0037, and the library's only randomness), `GalaxyLayout` (where the systems are and which of them a gate links, from one seed, plus `RouteAcrossGates`, the breadth-first search a fleet's voyage takes its next hop from — ADR 0055, ADR 0069), `UniverseSnapshot` (the wire format, ADR 0008) and `Publisher` (the fan-out to N subscribers, ADR 0030). Depends on NeuronCore only. |
...
**NeuronServer gates in the same step**, promoted on 2026-08-30 after two clean runs under the
runner's own LLVM 22 — it arrived advisory, exactly as this rule asks, and the exit line grew its
count. The next project after it is NeuronCore, whose files slice 22 already swept.

clang-tidy itself landed non-blocking and was promoted two runs later, which is the process
working. **A finding in GameLogic or NeuronServer is therefore yours**: both came back clean, so
anything the step reports there arrived
