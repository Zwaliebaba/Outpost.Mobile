...
{
// The sector pair behind UniversePos: the carry, the invariant it maintains, and the two consumers
...
public:
  ...
  {
    Game::UniversePos pos = Game::LocalPos(0.0f, 0.0f);
  ...
  TEST_METHOD(TheCarryIsExactAndReversible)
  {
    ...
    const Game::UniversePos start = Game::LocalPos(123.5f, -4321.25f);
    Game::UniversePos pos = start;
  ...
  TEST_METHOD(DistanceDoesNotCareWhereTheBoundaryIs)
  {
    ...
    Game::UniversePos insideA = Game::LocalPos(4000.0f, 0.0f);
    Game::UniversePos insideB = insideA;
    Game::Translate(insideB, 10.0f, 0.0f);

    Game::UniversePos straddleA = Game::LocalPos(Game::SECTOR_SIZE_METRES - 5.0f, 0.0f);
    Game::UniversePos straddleB = straddleA;
  ...
  {
    Game::UniversePos before = Game::LocalPos(Game::SECTOR_SIZE_METRES - 10.0f, 0.0f);
    Game::UniversePos after = before;
    Game::Translate(after, 20.0f, 0.0f);

    const Game::UniversePos middle = Game::Lerp(before, after, 0.5f);
  ...
  TEST_METHOD(TheIndexFindsANeighbourOverTheBorder)
  {
    ...
    Game::UniversePos west = Game::LocalPos(Game::SECTOR_SIZE_METRES - 5.0f, 0.0f);
    Game::UniversePos east = west;
  ...
  TEST_METHOD(APathIsFoundAcrossASectorBoundary)
  {
    ...
    Game::UniversePos obstacle = Game::LocalPos(Game::SECTOR_SIZE_METRES, 0.0f);
    ...
    Game::UniversePos from = obstacle;
    Game::Translate(from, -600.0f, 0.0f);
    Game::UniversePos to = obstacle;
  ...
  TEST_METHOD(AUniverseStepsTheSameWhereverItSits)
  {
    ...
    const auto run = [](const Game::UniversePos& _origin)
    {
      Game::Universe universe;
      Game::UniversePos left = _origin;
      Game::Translate(left, -200.0f, 0.0f);
      Game::UniversePos right = _origin;
      Game::Translate(right, 200.0f, 0.0f);

      const Game::ShipId a = universe.SpawnShip(left, 0.0f, static_cast<std::uint32_t>(Game::HullId::Corvette));
      const Game::ShipId b = universe.SpawnShip(right, 0.0f, static_cast<std::uint32_t>(Game::HullId::Corvette));
      const Game::ShipId both[] = {a, b};

      Game::UniversePos target = _origin;
      Game::Translate(target, 0.0f, 900.0f);
      universe.IssueMoveOrder(both, target, false, 0.0f);

      for (int tick = 0; tick < 600; ++tick)
        universe.Step();

      std::vector<float> offsets;
      for (const Game::ShipState& ship : universe.Ships())
      {
        offsets.push_back(Game::OffsetX(_origin, ship.posUniverse));
        offsets.push_back(Game::OffsetZ(_origin, ship.posUniverse));
      }
      return offsets;
    };

    Game::UniversePos distant = Game::LocalPos(0.0f, 0.0f);
