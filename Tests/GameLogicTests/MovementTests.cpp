...
public:
  ...
  {
    Game::World world;
    const Game::ShipId ship = world.SpawnShip(Game::LocalPos(0.0f, 0.0f), 0.0f, 0);

    const Game::ShipId order[] = {ship};
    world.IssueMoveOrder(order, Game::LocalPos(0.0f, 300.0f), false, 0.0f);
    ...
    const float dz = 300.0f - WorldZ(state.posWorld);
  ...
  TEST_METHOD(AShipComesToRestOnceIdle)
  {
    ...
    const Game::ShipId order[] = {ship};
    world.IssueMoveOrder(order, Game::LocalPos(120.0f, 120.0f), false, 0.0f);
  ...
  TEST_METHOD(AnOrderedFacingIsHeldAfterArrival)
  {
    ...
    world.IssueMoveOrder(order, Game::LocalPos(0.0f, 200.0f), true, facing);
  ...
  TEST_METHOD(TheSameOrderProducesTheSameRun)
  {
    ...
    {
      Game::World world;
      world.SpawnShip(Game::LocalPos(-260.0f, 700.0f), 0.0f, static_cast<std::uint32_t>(Game::HullId::Structure));
      ...
      for (int i = 0; i < 5; ++i)
        ships.push_back(
          world.SpawnShip(Game::LocalPos(static_cast<float>(i) * 240.0f - 480.0f, 0.0f), 0.0f, static_cast<std::uint32_t>(fleet[i])));

      world.IssueMoveOrder(ships, Game::LocalPos(200.0f, 1350.0f), true, 1.2f);
  ...
  TEST_METHOD(AFormationOrderSpreadsShipsOut)
  {
    ...
    std::vector<Game::ShipId> ships;
    for (int i = 0; i < 4; ++i)
      ships.push_back(world.SpawnShip(Game::LocalPos(static_cast<float>(i) * 50.0f, 0.0f), 0.0f, 0));

    world.IssueMoveOrder(ships, Game::LocalPos(0.0f, 400.0f), false, 0.0f);
