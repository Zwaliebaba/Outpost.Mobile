...
// The body every test below builds: heights above zero outside its one continent as well as inside
// it. It used to have a wet twin, whose sea floor was culled and whose coast was dipped under a
// sphere of water; that went with the sea (Design/Decisions/0026) and this is the one that is left.
...
// The build of the test description through AxisRamp, byte for byte. **This is the replay key for
// the whole mesh**, the way PINNED_HEIGHT_METRES is for the field. If it moves, every body the game
// has drawn is a different body, and the pull request says why -- it is not a number to update until
// a test goes green.
//
// It has moved once, when the vertex was packed (Decisions/0019). It did *not* move when the ocean
// arrived and it did not move when the ocean was taken away again: a dry body never took the shore
// dip or the sea-floor cull, so the whole of that feature and the whole of its removal are provably
// outside the path that draws a rock. That is what this literal is worth, and it is why the wet
// twin that used to sit beside it could be deleted without touching this one.
...
public:
  ...
  TEST_METHOD(TheWholeVertexListIsPinned)
  {
    ...
    Assert::AreEqual(PINNED_DRY_VERTEX_HASH, HashVertices(terrain), L"the mesh has changed; see PINNED_DRY_VERTEX_HASH");
    Assert::AreEqual(50.0f, stats.maxHeightMetres, 0.5f, L"the reported maximum height is not the field's");

    // Every cell is drawn: nothing culls any more, so the triangle count is the grid itself.
    constexpr std::uint32_t TRIANGLES = Neuron::CUBE_FACE_COUNT * 8 * 8 * 2;
    Assert::AreEqual(TRIANGLES, stats.trianglesEmitted, L"a cell was not drawn");
  }
};
} // namespace NeuronClientTests
