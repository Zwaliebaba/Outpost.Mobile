...
hold no game semantics and this struct names an interest radius and a shard; `NeuronServer` knows
...
- `Outpost/OutpostApp.cpp` — `OUTPOST_QUIC_PORT` and its three uses; `SpawnStartingFleet`, where
  `ConfigureShard` already is; `OpenLink`.
...
- **One subscriber, shard 0.** The file can express more; the executable still opens one link.
