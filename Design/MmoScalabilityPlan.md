...
entity has an identity that outlives the World that minted it. Slice 18 opens the client track's
...
**What is left is slices 17, 19 and 20, plus slice 23's promotion commit.**
...
- **The wire's sector index narrowed to i32**, which the scope did not ask for. Sixteen bytes of
  ...
  `WorldPos` is untouched. That is ADR 0046, and the plan's table forecast no record for this slice:
...
number is measured and recorded in ADR 0046 so it is not rediscovered — but they are neither
...
against the candidate for two reasons ADR 0047 carries: a slot-and-generation id *looks* derivable
...
- **`FactionId` stays u8**, which answers §4 decision 4. Widening the id alone buys nothing while
  ...
  byte. The premise was "if player corporations become factions", and ADR 0047 says they should not:
