...
**What is left is slices 17 through 20, plus slice 23's promotion commit.**
...
3. ~~**How a dedicated server is told what to be**~~ — **taken 2026-08-30: a configuration file, read
   by the composition root alone; built as slice 24, ADR 0045.** This does not bend AGENTS.md §5: that rule bans `argv` and the
...
every mesh and font already uses — and it carries six keys, each with a consumer today: `port`,
`backlog`, `shard`, `interestRadiusMetres`, `interestUpdateEveryTicks`, `ordersPerTick`. Work order:
[`ServerConfig-work-order.md`](Archive/ServerConfig-work-order.md); ADR 0045.
