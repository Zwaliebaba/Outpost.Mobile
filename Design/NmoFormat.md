...
**Status: proposed; slice 1 (format spec, codec, Blender add-on, tests) accompanies this document.
No engine code changes yet.** §14 lists the slices.
...
- Slice 2 ports the malformed-file suite to C++ against the same bytes, regenerated by
  `Tools/NmoFixture.py` — the fixture is *built by* a committed script, never committed as a
  binary, honouring "nothing generated is committed". (How slice 2's VS test gets the bytes on a
  Windows runner — checked-in `.nmo` asset versus a generation step — is decided in that slice's
  work order; §15 Q3.)
...
## 15. Open questions (for the owner; defaults chosen so work can proceed)

1. **Dialect unification.** v2.0 deliberately diverges from the Interstellar Outpost NMO (§3). If
   the two trees should instead share one format and one Blender add-on, say so: the deltas fold
   back behind that tree's `MatrixKeys`/CMO-fidelity constraints, and this document shrinks to a
   delta note. Default: separate dialects, shared lineage, distinct magic.
2. **Marker kind vocabulary.** `Exhaust`, `NavLight`, `Gun` are defined; the mechanism is open.
   Any kinds worth naming now (`DockingPort`, `Hardpoint`, `ShieldEmitter`) cost one table row
   each — naming them early only helps tools agree. Default: the three required kinds.
3. **Slice 2 fixture transport.** Checked-in `.nmo` test asset (simple, but a committed binary a
   script can regenerate) versus regenerating via python in the Windows test run (no binary, but
   a python dependency in that suite). Default recommendation: check the small fixture in beside
   the `.dds` fonts, with its generator committed alongside.
4. **Texture slots.** Four names per material are carried unused (§5.5) purely to keep a future
   texture pipeline out of major-version territory. If the flat-colour look is a permanent
   commitment, they could drop to zero. Default: keep four.
5. **Blink policy.** `NavLight` blink lives in marker params (period/phase) — authored per light.
   If blinking should be a fleet-wide style instead, the params stay zero and `ViewTuning.h`
   constants rule. Default: per-marker, zero meaning "steady".
