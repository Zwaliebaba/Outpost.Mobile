# 0043 — A server is told what to be by a file the composition root reads
...
how a *dedicated* server would be told what to be — its port, how many connections it carries, what
one subscriber is sent — and the owner took it on 2026-08-30: **a
...
Five keys, each with a consumer that exists today: `port`, `backlog`, `interestRadiusMetres`,
`interestUpdateEveryTicks`, `ordersPerTick`. A `shard` key joins them with the identity slice
(MmoScalabilityPlan.md slice 16), whose `World::ConfigureShard` is its only consumer; a key with no
consumer is a lie the day one is added, so it is not carried ahead of it. Every default is taken from the
...
- **Leave it compiled in.** What the tree did. Rejected because it is what gates a second process:
  two servers on one machine need two ports, and — once identities are shard-scoped — two shard
  ids, or they issue colliding ones. Neither is a rebuild.
...
- **JSON, TOML, or YAML.** Every one of them is a dependency (AGENTS.md §5's approval rule) or a
  generator (R7), for a format that has five scalar keys. The hand-written reader is fifty lines and
...
- **Put the parser in a library.** `NeuronCore` may hold no game semantics and this struct names an
  interest radius; `NeuronServer` knows nothing about the game it ticks. ADR 0002's
